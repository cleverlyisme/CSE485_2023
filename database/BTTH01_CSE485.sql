...
--
-- Host: 127.0.0.1
-- Generation Time: Feb 27, 2023 at 03:41 AM
...
--
DELIMITER $$
CREATE TRIGGER `tg_CapNhatTheLoai_SuaBaiViet` AFTER UPDATE ON `baiviet` FOR EACH ROW BEGIN IF NEW.ma_tloai != OLD.ma_tloai THEN  UPDATE theloai SET SLBaiViet = SLBaiViet + 1 WHERE ma_tloai = NEW.ma_tloai; UPDATE theloai SET SLBaiViet = SLBaiViet - 1 WHERE ma_tloai = OLD.ma_tloai; END IF; END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `tg_CapNhatTheLoai_ThemBaiViet` AFTER INSERT ON `baiviet` FOR EACH ROW BEGIN UPDATE theloai SET SLBaiViet = SLBaiViet + 1 WHERE theloai.ma_tloai = NEW.ma_tloai; END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `tg_CapNhatTheLoai_XoaBaiViet` AFTER DELETE ON `baiviet` FOR EACH ROW BEGIN  UPDATE theloai SET SLBaiViet = SLBaiViet - 1 WHERE ma_tloai = OLD.ma_tloai; END
$$
DELIMITER ;
...
CREATE TABLE `theloai` (
  ...
  `SLBaiViet` int(10) UNSIGNED NOT NULL DEFAULT 0
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_music`  AS   (select `bv`.`ma_bviet` AS `ma_bviet`,`bv`.`tieude` AS `tieude`,`bv`.`ten_bhat` AS `ten_bhat`,`tl`.`ten_tloai` AS `ten_tloai`,`bv`.`tomtat` AS `tomtat`,`bv`.`noidung` AS `noidung`,`tg`.`ten_tgia` AS `ten_tgia`,`bv`.`ngayviet` AS `ngayviet`,`bv`.`hinhanh` AS `hinhanh` from ((`baiviet` `bv` left join `tacgia` `tg` on(`bv`.`ma_tgia` = `tg`.`ma_tgia`)) left join `theloai` `tl` on(`bv`.`ma_tloai` = `tl`.`ma_tloai`)))  ;
